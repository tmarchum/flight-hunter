...
> מאומת מול הקוד בפרודקשן: `1b56730` (2026-07-09).
...
| **🌍 לאן שהכי זול** (`explore`) | מוצא + תאריך + חלון שעות + תקציב (בלי יעד) | 6 היעדים הזולים בחלון ההמראה | `service_price` |

### מסלול Explore — פרטי מימוש (§4.5)
מנוע דו-שלבי: (1) Skyscanner `search-everywhere` → ~100 יעדים מועמדים במחיר
אינדיקטיבי; (2) חיפוש SerpApi אמיתי על ~12 המובילים, מסונן לחלון ההמראה
ולתקציב, ממוין לפי מחיר אמיתי-להזמנה. **שלב 2 קריטי** — המחיר האינדיקטיבי
($21 קפריסין) מטעה לחלוטין לעומת האמיתי בתאריך/שעה ($324 לרנקה 08:10), אותה
מלכודת כמו באג ה-Kiwi הישן. מצטטים רק מחירי שלב 2. `COUNTRY_TO_AIRPORT` ממפה
קוד-מדינה לשדה הראשי. עמודות `depart_time_from`/`depart_time_to` (HH:MM).
