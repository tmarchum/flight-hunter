...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      {
        const isBeatAck = request.type === "beat";
        const price = settings.service_price || "249";
        let ack = `שלום ${request.name} 👋\n\n✅ הבקשה שלך התקבלה!\n\n`;
        ack += `✈️ ${heCity(request.from_iata)} → ${heCity(request.to_iata)}\n`;
        ack += `🔄 ${tripLabel(request)}\n`;
        ack += `📅 ${request.depart_date}${request.return_date ? " — " + request.return_date : ""}\n`;
        ack += `👥 ${request.adults} מבוגרים${request.children ? " + " + request.children + " ילדים" : ""}\n`;
        ack += isBeatAck ? `💰 המחיר שמצאת: $${request.customer_price_usd}\n` : `📊 סוג: דוח מחקר\n`;
        ack += `\nהמערכת מחפשת עבורך כרגע בכל המקורות.\nנעדכן אותך ב-WhatsApp ברגע שנסיים! 🔍\n`;
        ack += `💰 עלות השירות: ₪${price} — רק אם הצלחנו\n\n_צייד טיסות ✈️_`;
        await sendWhatsApp(settings, request.whatsapp, ack);
      }
